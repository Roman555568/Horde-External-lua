function EFFECT:Init(data)
    ...
    local muzzle = wpn.MuzzleEffect_first
    local overridemuzzle = wpn:GetBuff_Override("Override_MuzzleEffect_first")

    local gmmuzzle = wpn:GetBuff_Override("Override_GMMuzzleEffect") or wpn.GMMuzzleEffect

    muzzle = overridemuzzle or muzzle

    local muzzle2 = wpn.MuzzleEffect_second
    local overridemuzzle2 = wpn:GetBuff_Override("Override_MuzzleEffect_second")

    muzzle2 = overridemuzzle2 or muzzle2
    ...
    if gmmuzzle then
        ...
        if muzzle2 then
            ParticleEffectAttach(muzzle2, PATTACH_POINT_FOLLOW, mdl, att)
        end
    end
