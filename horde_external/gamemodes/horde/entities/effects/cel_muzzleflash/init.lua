...
function EFFECT:Init(data)
	...
	for i = 1, 2 do
		local particle = emitter:Add("particle/warp_ripple", self.vOffset)
		...
		if (particle) then
			...
			particle:SetEndSize(25 * (halofac * 0.8 + 0.2))
	...
	for i = 0, 5 do
		local particle = emitter:Add("particle/smokesprites_0006", self.vOffset + (dir * 0.6 * i))
		...
		if (particle) then
			...
			particle:SetEndSize(math.max(9.4 - 0.55 * i, 1) * 1.25)
			particle:SetRoll(math.rad(math.Rand(0, 360)))
			particle:SetRollDelta(math.rad(math.Rand(-40, 40)))
			particle:SetColor(255, 161, 0)
			particle:SetLighting(false)
			particle.FollowEnt = self.WeaponEnt
			particle.Att = self.Attachment
			--TFA.Particles.RegisterParticleThink(particle, TFA.Particles.FollowMuzzle)
	...
	for i = 0, 4 do
		local particle = emitter:Add("effects/energy_electric", self.vOffset + dir * math.Rand(6, 10))
	...
	for i = 0, 4 do
		local particle = emitter:Add("effects/energy_electric", self.Position)
